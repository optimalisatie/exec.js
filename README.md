...
The `allow-scripts` and `allow-same-origin` sandbox parameters and the `unsafe-eval` CSP parameter are enabled by default. Sandbox isolation and CSP are disabled by default.
